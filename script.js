...
function getCurrentReferenceData() {
  return {
...
function applyReferenceDisplay(data) {
  document.getElementById("displayHbaRight").innerText = formatUSD(data.hba);
  document.getElementById("displayHba1Right").innerText = formatUSD(data.hba1);
  document.getElementById("displayHba2Right").innerText = formatUSD(data.hba2);
  document.getElementById("displayHba3Right").innerText = formatUSD(data.hba3);
  document.getElementById("latestHbaPeriodDisplay").innerText = `Period: ${data.period || "-"}`;
}
...
function syncManualReferenceRealtime() {
  if (document.getElementById("hbaReferenceMode").value !== "MANUAL") return;

  const manualData = getCurrentReferenceData();
...
function toggleHbaReferenceMode() {
  ...
  const applyWrap = document.getElementById("applyReferenceWrap");
...
function updateSingleIndexLabel() {
  ...
  document.getElementById("singleIndexValueLabel").innerText = `${formatIndexLabel(indexName)} Value`;
}

function updateBlendedIndexLabels() {
  document.getElementById("leg1ValueLabel").innerText =
    `${formatIndexLabel(document.getElementById("leg1Index").value)} Value`;
  document.getElementById("leg2ValueLabel").innerText =
    `${formatIndexLabel(document.getElementById("leg2Index").value)} Value`;
}
...
function togglePricingSections() {
  ...
  document.getElementById("singleIndexSection").classList.add("hidden");
  document.getElementById("blendedIndexSection").classList.add("hidden");
  document.getElementById("fixedPriceSection").classList.add("hidden");

  if (pricingType === "SINGLE_INDEX") {
    document.getElementById("singleIndexSection").classList.remove("hidden");
  } else if (pricingType === "BLENDED_2_INDEX") {
    document.getElementById("blendedIndexSection").classList.remove("hidden");
  } else {
    document.getElementById("fixedPriceSection").classList.remove("hidden");
  }
}
...
function getRoyaltyRate({ permitType, hpbMode, hba, gcv }) {
  if (permitType === "IUPK") {
    if (hpbMode === "CAP70" || hpbMode === "CAP90") return 14;
    if (hba < 70) return 15;
    if (hba < 120) return 18;
    if (hba < 140) return 19;
    if (hba < 160) return 22;
    if (hba < 180) return 25;
    return 28;
  }
  ...
  if (gcv < 5200) {
    if (isCap) return 8.5;
    if (hba < 70) return 7;
    if (hba < 90) return 8.5;
    return 11.5;
  }

  if (isCap) return 11.5;
  if (hba < 70) return 9.5;
  if (hba < 90) return 11.5;
  return 13.5;
}
...
function calculate() {
  ...
  const hbaReference = parseFloat(document.getElementById("manualHba").value) || 0;
  const transhipmentFreight = parseFloat(document.getElementById("transhipmentFreight").value) || 0;
  ...
  function priceCAP(hbaCap) {
    if (gcv > 4200) {
      return hbaCap * (gcv / 6322) * ((100 - tm) / (100 - 8)) -
        (((ts - 0.8) * 4) + ((ash - 15) * 0.4));
    } else {
      const base = hbaCap * (4200 / 6322) *
        ((100 - 35) / (100 - 8 / ((((100 - 8) / (100 - 35)) * 35 + (100 - 8)) / 100)));

      if (tm < 40) {
        return base * (gcv / 4200) * ((100 - tm) / (100 - 35)) *
  ...
  function price6000(hba) {
    return hba * (gcv / 6322) * ((100 - tm) / (100 - 12.26)) -
      (((ts - 0.66) * 4) + ((ash - 7.94) * 0.4));
  }

  function price5300(hba1) {
    return hba1 * (gcv / 5300) * ((100 - tm) / (100 - 21.32)) -
      (((ts - 0.75) * 4) + ((ash - 6.04) * 0.4));
  }

  function price4100(hba2) {
    return hba2 * (gcv / 4100) * ((100 - tm) / (100 - 35.73)) -
      (((ts - 0.23) * 4) + ((ash - 3.9) * 0.4));
  }

  function price3400(hba3) {
    return hba3 * (gcv / 4100) *
      ((100 - tm) / (100 - 35.73 / ((((100 - 35.73) / (100 - tm)) * tm + (100 - 35.73)) / 100))) -
      (((ts - 0.23) * 4) + ((ash - 3.9) * 0.4));
  }

  function priceSub3400(hba3) {
    return hba3 * (gcv / 3400) *
      ((100 - tm) / (100 - 44.3 / ((((100 - 44.3) / (100 - tm)) * tm + (100 - 44.3)) / 100))) -
  ...
  function calculateSingleIndexSellingPrice() {
    ...
    if (premDiscPosition === "BEFORE_PRORATE") {
      ...
      baseFormula = `(${formatIndexLabel(indexName)} ${premDiscLabel} ${premDiscValue}) × ${contractGcv} / ${typicalIndexGcv}`;
    } else {
      const proratedIndex = typicalIndexGcv ? (indexValue * contractGcv / typicalIndexGcv) : 0;
      basePrice = proratedIndex + premDiscSigned;
      baseFormula = `(${formatIndexLabel(indexName)} × ${contractGcv} / ${typicalIndexGcv}) ${premDiscLabel} ${premDiscValue}`;
    }

    const adjPrice = contractGcv ? (basePrice * gcv / contractGcv) : 0;
    const adjFormula = contractGcv ? `${formatUSD(basePrice)} × ${gcv} / ${contractGcv}` : "-";

    return { mode: "Single Index", basePrice, adjPrice, baseFormula, adjFormula };
  }
  ...
  function calculateBlended2SellingPrice() {
    ...
    if (premDiscPosition === "BEFORE_PRORATE") {
      leg1Base = typicalIndexGcv1 ? (w1 * (leg1Value + leg1PremDiscSigned)) * contractGcv / typicalIndexGcv1 : 0;
      leg2Base = typicalIndexGcv2 ? (w2 * (leg2Value + leg2PremDiscSigned)) * contractGcv / typicalIndexGcv2 : 0;
      ...
      leg1Base = typicalIndexGcv1 ? ((w1 * leg1Value) * contractGcv / typicalIndexGcv1) + leg1PremDiscSigned : 0;
      leg2Base = typicalIndexGcv2 ? ((w2 * leg2Value) * contractGcv / typicalIndexGcv2) + leg2PremDiscSigned : 0;
    ...
    return { mode: "Blended 2 Index", basePrice, adjPrice, baseFormula, adjFormula };
  }
  ...
  function calculateFixedPrice() {
    ...
    const adjFormula = contractGcv ? `${formatUSD(fixedBasePrice)} × ${gcv} / ${contractGcv}` : "-";
...
function exportToPDF() {
  ...
  document.getElementById("pdfTimestamp").innerText = `Generated on ${now.toLocaleString("en-US")}`;
  document.getElementById("pdfGcv").innerText =
    document.getElementById("gcv").value
      ...
      : "-";
  document.getElementById("pdfTm").innerText = document.getElementById("tm").value ? `${document.getElementById("tm").value} %` : "-";
  document.getElementById("pdfTs").innerText = document.getElementById("ts").value ? `${document.getElementById("ts").value} %` : "-";
  document.getElementById("pdfAsh").innerText = document.getElementById("ash").value ? `${document.getElementById("ash").value} %` : "-";
  ...
  document.getElementById("pdfSelectedFormula").innerText = document.getElementById("selectedFormula").innerText;
  document.getElementById("pdfHpb").innerText = document.getElementById("result").innerText;
  document.getElementById("pdfRoyaltyRate").innerText = document.getElementById("royaltyRateResult").innerText;
  document.getElementById("pdfRoyaltyValue").innerText = document.getElementById("royaltyValueResult").innerText;
  document.getElementById("pdfPricingMode").innerText = document.getElementById("pricingModeResult").innerText;
  document.getElementById("pdfTypicalContractGcv").innerText = document.getElementById("typicalContractGcvResult").innerText;
  document.getElementById("pdfBaseFormula").innerText = document.getElementById("basePriceFormula").innerText;
  document.getElementById("pdfBasePrice").innerText = document.getElementById("basePriceResult").innerText;
  document.getElementById("pdfAdjFormula").innerText = document.getElementById("adjPriceFormula").innerText;
  document.getElementById("pdfAdjPrice").innerText = document.getElementById("adjPriceResult").innerText;
...
function bindRealtimeEvents() {
  [
    ...
    if (!el) return;
    el.addEventListener("input", handleRealtimeUi);
    el.addEventListener("change", handleRealtimeUi);
  });
}

function handleRealtimeUi(event) {
  const id = event?.target?.id;

  if (id === "pricingType") togglePricingSections();
  if (id === "singleIndexName") updateSingleIndexLabel();
  if (id === "leg1Index" || id === "leg2Index") updateBlendedIndexLabels();
  if (id === "hbaReferenceMode") toggleHbaReferenceMode();
